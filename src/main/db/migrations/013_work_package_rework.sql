-- =============================================================================
-- Work-package coding, corrected: the mapping key is the line's own
-- identity, not its GL account's pattern or its WBS location.
--
-- 011_work_packages.sql shipped a GLOB rule table keyed on
-- (cost_element, wbs) — wrong, and by the time that was discovered, real
-- local databases had already applied it, so it could not simply be edited
-- in place (schema_migration already records versions 11/12 as applied,
-- and migrate() never re-runs an applied version). This migration is the
-- forward fix instead, reachable from either starting point:
--   - a database that already ran the original 011/012 (work_package_rule,
--     no group_label, no per-line mapping tables, no fact_budget.work_package)
--   - a brand new database about to run 011/012 for the first time, which
--     still install that same original shape before this migration corrects it
--
-- Concretely, per cost type:
...
--
-- Never edit this migration in place either, once it has shipped anywhere —
-- carry any further correction forward in a new migration the same way.
-- =============================================================================

ALTER TABLE dim_work_package ADD COLUMN group_label TEXT;

-- Safe even if a database already has some other row coded 'INDIRECT' from
-- earlier testing (INSERT OR IGNORE leaves an existing row untouched).
...
-- Nothing above references it any more.
DROP TABLE work_package_rule;
